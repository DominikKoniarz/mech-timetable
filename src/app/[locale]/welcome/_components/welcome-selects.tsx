			...
			<FormField
				...
				render={({ field }) => (
					<FormItem>
						...
						<Select
							onValueChange={isFirstRender ? undefined : field.onChange}
							defaultValue={isFirstRender ? undefined : field.value}
				...
				render={({ field }) => (
					<FormItem>
						...
						>
							<FormControl>
								<SelectTrigger className="w-full data-[placeholder]:text-white">
									<SelectValue placeholder={COMPUTER_LAB_GROUPS.K01} />
								</SelectTrigger>
							</FormControl>
							<SelectContent>
								{Object.values(COMPUTER_LAB_GROUPS).map((group) => (
									<SelectItem key={group} value={group}>
										{group}
									</SelectItem>
								))}
				...
				render={({ field }) => (
					<FormItem>
						...
						>
							<FormControl>
								<SelectTrigger className="w-full data-[placeholder]:text-white">
									<SelectValue placeholder={PROJECT_GROUPS.P01} />
								</SelectTrigger>
							</FormControl>
							<SelectContent>
